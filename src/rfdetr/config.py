...
def _default_train_devices() -> int:
    """Return the default number of devices for training on this machine."""
    env_value = os.environ.get("RFDETR_TRAIN_DEVICES")
    if env_value:
        try:
            devices = int(env_value)
        except ValueError:
            warnings.warn(
                f"Ignoring invalid RFDETR_TRAIN_DEVICES={env_value!r}; expected a positive integer.",
                UserWarning,
                stacklevel=2,
            )
        else:
    ...
    if DEVICE == "cuda":
        try:
            ...
            return 1
        return 2 if cuda_devices >= 2 else 1
    return 1
...
class ModelConfig(BaseConfig):
    ...
    dinov2_backbone_ckpt: Optional[str] = None
    ...
    @field_validator("pretrain_weights", "dinov2_backbone_ckpt", mode="after")
    @classmethod
    def expand_path(cls, v: Optional[str]) -> Optional[str]:
        """Expand and resolve checkpoint paths.
...
class TrainConfig(BaseModel):
    ...
    strategy: str = "auto"
    devices: Union[int, str] = Field(default_factory=_default_train_devices)
