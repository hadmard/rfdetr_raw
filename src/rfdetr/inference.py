...
from rfdetr.models.weights import apply_lora, load_dinov2_backbone_weights, load_pretrain_weights
...
def _build_model_context(model_config: ModelConfig) -> ModelContext:
    ...
    if model_config.dinov2_backbone_ckpt is not None:
        if model_config.pretrain_weights is not None:
            raise ValueError(
                "dinov2_backbone_ckpt requires pretrain_weights=None so full RF-DETR checkpoint weights "
                "are not loaded."
            )
        load_dinov2_backbone_weights(nn_model, model_config.dinov2_backbone_ckpt)
