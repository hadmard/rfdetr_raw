...
class TestBuildTrainerDDPFields:
    ...
    def test_default_devices_is_1_without_cuda(self, tmp_path):
        """Default TrainConfig.devices must produce devices=1 when CUDA is unavailable."""
        import unittest.mock as mock
        ...
        with (
            mock.patch("rfdetr.config.DEVICE", "cpu"),
            mock.patch("rfdetr.training.trainer.Trainer", side_effect=_fake_trainer),
        ):
            tc = _tc(tmp_path, use_ema=False)
            build_trainer(tc, _mc())
    ...
    def test_default_devices_is_2_with_two_cuda_devices(self, tmp_path):
        """Default TrainConfig.devices uses two GPUs when at least two CUDA devices are visible."""
        import unittest.mock as mock
        ...
        with (
            mock.patch("rfdetr.config.DEVICE", "cuda"),
            mock.patch("rfdetr.config.torch.cuda.device_count", return_value=2),
            mock.patch("rfdetr.training.trainer.Trainer", side_effect=_fake_trainer),
        ):
            tc = _tc(tmp_path, use_ema=False)
            build_trainer(tc, _mc())
